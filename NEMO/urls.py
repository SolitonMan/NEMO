...
urlpatterns = [
	...
	re_path(r'^enable_tool/(?P<tool_id>\d+)/user/(?P<user_id>\d+)/project/(?P<project_id>\d+)/staff_charge/(?P<staff_charge>(true|false))/billing_mode/(?P<billing_mode>(true|false))/end_scheduled_outage/(?P<end_scheduled_outage>\d+)/$', tool_control.enable_tool, name='enable_tool_configuration'),
