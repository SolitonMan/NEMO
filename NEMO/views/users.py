...
def user_requests(request):
	...
	mcl_services = ServiceType.objects.filter(core=mcl_core, active=True).exclude(name__in=requirement_names).order_by('logical_order','name')
	nano_core = Core.objects.get(name='Nanofabrication Facility')
	nano_services = ServiceType.objects.filter(core=nano_core, active=True).exclude(name__in=requirement_names).order_by('logical_order','name')
	twodcc_core = Core.objects.get(name='2DCC')
	twodcc_services = ServiceType.objects.filter(core=twodcc_core, active=True).exclude(name__in=requirement_names).order_by('logical_order','name')
