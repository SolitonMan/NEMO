...
def remote_work(request):
	...
	if operator:
		...
		area_access_records = area_access_records.filter((Q(customer_id=operator.id) & Q(customer_id__isnull=False)) | (Q(user_id=operator.id)) | Q(customers__id=operator.id))
