...
def work_order_transactions(request, work_order_id):
	...
	usage_events = UsageEvent.objects.filter(end__gte=dstart, customers__id=work_order.customer.id, active_flag=True, work_order_transaction=None)
	uep = UsageEventProject.objects.filter(usage_event__in=usage_events, customer=work_order.customer)

	# get staff charges
	staff_charges = StaffCharge.objects.filter(end__gte=dstart, customers__id=work_order.customer.id, active_flag=True, work_order_transaction=None)
	scp = StaffChargeProject.objects.filter(staff_charge__in=staff_charges, customer=work_order.customer)

	# get area access records
	area_access_records = AreaAccessRecord.objects.filter(end__gte=dstart, customers__id=work_order.customer.id, active_flag=True, work_order_transaction=None)
	aarp = AreaAccessRecordProject.objects.filter(area_access_record__in=area_access_records, customer=work_order.customer)
