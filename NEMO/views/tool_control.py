...
def tool_control(request, tool_id=None, qualified_only=None, core_only=None):
	...
	if request.device == 'desktop':
		dictionary['rendered_tool_tree_html'] = ToolTree().render(None, {'tools': tools}, selected_tool_id=tool_id)
...
#and current_usage_event.start >= current_reservation.start-timedelta(0,0,0,0,15,0,0) and current_usage_event.start <= current_reservation.start+timedelta(0,0,0,0,15,0,0)) or (current_reservation is not None and current_usage_event is None):
	...
	if request.user.is_staff or ProbationaryQualifications.objects.filter(user=request.user, tool=tool, disabled=False).exists():
...
def create_usage_event(request):
	if request.user.core_ids.all().count() > 0 and not request.user.is_superuser:
		qualified_tool_ids = ProbationaryQualifications.objects.filter(user=request.user, disabled=False).values_list('tool_id', flat=True)
		tools = Tool.objects.filter(Q(core_id__in=request.user.core_ids.all()) | Q(id__in=qualified_tool_ids))
	else:
